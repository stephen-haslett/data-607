...
fetch_training_file_data <- function(directory, type) {
  ...
  training_emails$class <- type
  colnames(training_emails) <- c('message', 'message_type')
...
combined_dataset$message_type <- combined_dataframe$message_type
```
...
split <- sample.split(randomized_dataset$message_type, SplitRatio = 0.8)
